...
% For specifying matrix dimensions.
...
sound correspondences and dialects: bipartite spectral graph co-clustering.
...
/t/ (e.g. \textit{Tid}/\textit{Zeit} `time',
...
the Proto-Germanic and the modern data makes it possible
to\textemdash{}in addition to using patterns found in doculect-specific
sound correspondences\textemdash{}base the alignment on commonalities
...
$A_n = U\Sigma{}V^T$
($U~\in~\R^{m \times m}$,
$\Sigma{}~\in~\R_{\geq 0}^{m \times n}$ is a diagonal matrix with values in descending order,
$V~\in~\R^{n \times n})$
...
\vspace{2em}
\includestandalone[height=0.42\textheight]{figures/bsgc-context}
\end{adjustwidth}
...
A further approach would be to run clustering
